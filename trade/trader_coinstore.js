    ...
    return new Promise((resolve) => {
      coinstoreApiClient.ticker().then((markets) => {
        try {
          ...
          /**
           * JITOSOLUSDT is in pre-market: its ticker is all zeros, so decimals derived
           * from ticker digits above are wrong (0/0), making the bot round order amounts
           * and prices to integers. Override with the values configured by Coinstore:
           * price decimals 2, quantity decimals 2, minimum limit-order quantity 0.01.
           * Remove this block once the pair is publicly listed with live trading data.
           */
          if (result['JITOSOLUSDT']) {
            result['JITOSOLUSDT'].coin1Decimals = 2;
            result['JITOSOLUSDT'].coin2Decimals = 2;
            result['JITOSOLUSDT'].coin1Precision = 0.01;
            result['JITOSOLUSDT'].coin2Precision = 0.01;
            result['JITOSOLUSDT'].coin1MinAmount = 0.01;
          }
